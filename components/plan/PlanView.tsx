...
export function PlanView({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-slate-50 dark:bg-slate-950">
      <header className="h-20 px-8 flex items-center justify-between border-b bg-white dark:bg-slate-900 shadow-sm z-10 shrink-0">
         <div className="space-y-1">
            <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">{plan.projectName}</h1>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Architectural Build Plan</p>
         </div>
         <Button 
            onClick={approvePlan}
            disabled={!hasScrolledToBottom}
            className="bg-blue-600 hover:bg-blue-700 text-white font-black h-11 px-8 rounded-xl shadow-xl shadow-blue-500/20 disabled:opacity-50 disabled:grayscale"
         >
        ...
        <section className="max-w-4xl mx-auto space-y-8">
            ...
            <div className="space-y-6">
                <div className="flex items-center gap-4">
                    <h2 className="text-sm font-black text-slate-800 uppercase tracking-[0.3em] shrink-0">Selected Stack</h2>
            ...
            <div className="space-y-12">
                {plan.phases.map((phase: any, i: number) => (
                    <div key={i} className="space-y-6">
                         <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-xl bg-slate-900 flex items-center justify-center text-white font-black font-mono">
                                {i + 1}
                            </div>
                            <h2 className="text-xl font-black text-slate-900 tracking-tight">{phase.name}</h2>
                            <div className="h-px bg-slate-200 flex-1" />
                        </div>
            ...
            {!hasScrolledToBottom && (
                <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-white/80 dark:bg-slate-900/80 backdrop-blur px-6 py-3 rounded-full border shadow-xl flex items-center gap-3 animate-bounce">
                    <span className="text-xs font-bold text-slate-600 dark:text-slate-400">Scroll to confirm review</span>
